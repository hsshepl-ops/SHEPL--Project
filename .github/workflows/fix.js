// Overrides saveUser to use REST API so admin is never signed out
  ...
  try {
    ...
    if (userId) {
      ...
      await db.collection('users').doc(newUid).set({
        ...
        createdBy: S.user ? S.user.uid : ''
      });
      invalidateCache();
      await writeAudit('create', 'user', newUid, name, { role: role, email: email });
    ...
    if (ex.code === 'auth/email-already-in-use') {
      ...
      document.getElementById('user-err').textContent = 'Error: ' + (ex.message || JSON.stringify(ex));
    }
    btn.disabled = false;
    btn.textContent = userId ? 'Save Changes' : 'Create User';
  }
}
